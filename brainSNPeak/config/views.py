...
def api(snp_ids, brain_region="NA", exclude_dev_details=None):

    if exclude_dev_details == None or exclude_dev_details == "NA":
    ...
    if valid_input:
        try:
            ...
            for snp_id in snp_ids:
                ...
                if not len(res):
                    response_snps.append({"id": snp_id, "ocrs": []})
                else:
                    ...
                    if not path.exists(peak_info_path):
                        response_snps.append({"id": snp_id, "ocrs": "Can't find the path of mapped peak dataframe"})
                    else:
                        ...
                        response_snps.append({"id": snp_id, "ocrs": ocrs})
...
# receives only 1 SNP via GET method
def query_multiple_snp_ids(request, snp_ids, brain_region, exclude_dev_details):
    return api(snp_ids, brain_region, exclude_dev_details)

# receives multiple SNPs via POST method (POST forms can handle big amount of data)
def query_single_snp_id(request, snp_id, brain_region, exclude_dev_details):
    # capsulate into the multiple SNPs method
    return query_multiple_snp_ids(request, snp_id, brain_region, exclude_dev_details)


# receives multiple SNPs (POST forms can handle big amount of data)
def form_post_receiver(request):
    ...
    return query_multiple_snp_ids(request, snp_ids, brain_region, exclude_dev_details)
